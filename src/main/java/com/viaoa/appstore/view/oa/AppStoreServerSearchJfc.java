...
public class AppStoreServerSearchJfc {
    private static Logger LOG = Logger.getLogger(AppStoreServerSearchJfc.class.getName());
    
    private AppStoreServerSearchModel model;
    private JDialog dialog;
    protected AppStoreServerJfc jfcAppStoreServer;
    
    protected boolean bShowResults;
    protected boolean bMultiSelect;
    protected AppStoreServer appStoreServerSelected;
    protected boolean bSelected;
    
    public AppStoreServerSearchJfc() {
        this(null, true, false);
    }
    public AppStoreServerSearchJfc(AppStoreServerSearchModel model) {
        this(model, true, false);
    }
    public AppStoreServerSearchJfc(AppStoreServerSearchModel model, boolean bShowResults, boolean bMultiSelect) {
        this.model = model;
        this.bShowResults = bShowResults;
        this.bMultiSelect = bMultiSelect;
    }
    public AppStoreServerSearchJfc(boolean bShowResults) {
        this(null, bShowResults, false);
    }
    
    public AppStoreServerSearchModel getModel() {
        if (model == null) {
            model = new AppStoreServerSearchModel();
            model.getAppStoreServerSearch().setMaxResults(100); // default for UI searches
        }
        return model;
    }
    ...
    public JPanel createMainPanel() {
        ...
        if (cnt > 1) {
            ...
            Icon icon = Resource.getJarIcon("appStoreServer.gif");
            if (panSearch != null) {
    ...
    public JPanel createSearchFromPanel() {
        final Hub<AppStoreServer> hub = getModel().getSearchFromHub();
        ...
        if (bMultiSelect) table.setMultiSelectControlKey(true);
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    AppStoreServer appStoreServer = hub.getAO();
                    if (appStoreServer != null) onSelect(appStoreServer, hub);
                }
            });
        }
        return pan;
    ...
    public JPanel createMultiSelectPanel() {
        final Hub<AppStoreServer> hub = getModel().getMultiSelectHub();
        JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        JLabel lbl = new JLabel("AppStoreServers to add");
    ...
    public JPanel createSearchTreePanel() {
        // no tree properties defined in model, for AppStoreServer
        if (true || false) return null;
        // if (!bShowResults) return null;
        final JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        final AppStoreServerModel model = new AppStoreServerModel(ModelDelegate.getAppStoreServers().createSharedHub());
        if (bMultiSelect) {
        ...
        if (!bMultiSelect) {
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    AppStoreServer appStoreServer = model.getHub().getAO(); 
                    if (appStoreServer != null) onSelect(appStoreServer, model.getHub());
                }
            });
            pan.add(panx, BorderLayout.SOUTH);
        }
        ...
        SwingWorker<OATree, Void> sw = new SwingWorker<OATree, Void>() {
            ...
            protected OATree doInBackground() throws Exception {
                AppStoreServerJfc jfcAppStoreServer = new AppStoreServerJfc(model);
                OATree tree = jfcAppStoreServer.createSearchTree();
                tree.preload();
                return tree;
            }
            @Override
    ...
    protected JPanel createSearchResultsPanel() {
        ...
        final Hub<AppStoreServer> hub = getHub();
        OATable table = createReadOnlyTable(hub);
        OATableScrollPane spTable = new OATableScrollPane(table, 1);
    ...
    public OATextField createMaxResultsTextField() {
        OATextField txt = new OATextField(getModel().getAppStoreServerSearchHub(), AppStoreServerSearch.P_MaxResults, 5);
    ...
    public OATable createReadOnlyTable(final Hub<AppStoreServer> hubAppStoreServer) {
        Hub<AppStoreServer> hubx = new Hub(AppStoreServer.class);
        OATable table = new OATable(hubx) {
            @Override
            public void onDoubleClick() {
                if (AppStoreServerSearchJfc.this.getModel().getMultiSelectHub() != hubAppStoreServer) {
                    AppStoreServer appStoreServer = hubAppStoreServer.getAO();
                    if (appStoreServer != null) onSelect(appStoreServer, hubAppStoreServer);
                }
            }
        };
        table.setFilterMasterHub(hubAppStoreServer);
        table.setAllowDnD(false);
        table.setAllowSorting(true);
        table.addCounterColumn();
    
        if (bMultiSelect && getModel().getMultiSelectHub() != hubAppStoreServer) {
    ...
    protected void createTableColumns(OATable table) {
        getAppStoreServerJfc().createReadOnlyTableColumns(table);
    }
    public OAButton createSelectOneButton(final Hub<AppStoreServer> hub) {
        ...
        cmd.setToolTipText("Add selected AppStoreServer");
        OAButton.setup(cmd);
    ...
    public JButton createSelectManyButton() {
        ...
        cmd.setToolTipText("Add selected AppStoreServers");
    ...
    public OAButton createRemoveButton(Hub<AppStoreServer> hub) {
        OAButton cmd = new OAButton(hub, OAButton.REMOVE);
        cmd.setText("Remove");
        cmd.setup();
        cmd.setToolTipText("Remove selected AppStoreServer");
        return cmd;
    }
    public OAButton createRemoveAllButton(Hub<AppStoreServer> hub) {
        ...
        cmd.setup();
        cmd.setToolTipText("Remove all AppStoreServers from the select list");
        return cmd;
    }
    ...
    public AppStoreServerJfc getAppStoreServerJfc() {
        if (jfcAppStoreServer == null) {
            jfcAppStoreServer = new AppStoreServerJfc(new Hub<AppStoreServer>(AppStoreServer.class));
            OAObjectModel om = jfcAppStoreServer.getModel();
        ...
        return jfcAppStoreServer;
    }
    public OAButton createGotoEditButton(final Hub<AppStoreServer> hub) {
        OAButton cmd = new OAButton(hub, "Details ...", Resource.getJarIcon(Resource.IMG_Goto), OAButton.GOTO) {
            @Override
            public void afterActionPerformed() {
                getAppStoreServerJfc().getHub().setSharedHub(hub, true);
                getAppStoreServerJfc().getEditDialog(this).setVisible(true);
    ...
    public JDialog getDialog() {
        ...
        dialog = new JDialog(win, "App Store Server Search", ModalityType.APPLICATION_MODAL);
        ...
        dialog.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                AppStoreServerSearchJfc.this.bSelected = false;
                onShowDialog();
            }
            @Override
            public void windowClosing(WindowEvent e) {
                if (!AppStoreServerSearchJfc.this.bSelected) {
                    AppStoreServerSearchJfc.this.onCancel();
                }
                onHideDialog();
            }
        });
    ...
    protected void onReset() {
        getHub().cancelSelect();
        getHub().clear();
        getModel().getAppStoreServerSearch().reset();
    }
    
    protected void onSelect(AppStoreServer appStoreServer, Hub<AppStoreServer> hubFrom) {
        appStoreServerSelected = appStoreServer;
        bSelected = true;
        getHub().cancelSelect();
        getModel().onSelect(appStoreServer, hubFrom);
        if (dialog != null) dialog.setVisible(false);
    }
    protected void onSelect(Hub<AppStoreServer> hub) {
        bSelected = true;
        getHub().cancelSelect();
    ...
    protected void onSearch(final JComponent compFrom) {
        /*
        if (!getModel().getAppStoreServerSearch().isDataEntered()) {
            String msg = "Please enter selection data.";
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom), msg, "AppStoreServer Search", JOptionPane.WARNING_MESSAGE);
            return;
        }
        */
        ...
        dlgWait.setStatus("Selecting AppStoreServers ... please wait ...");
        dlgWait.setTitle("AppStoreServer Search");
        ...
        if (dlgWait.wasCancelled()) {
            ...
            String msg = "No AppStoreServers were found.";
            try {
                sw.get();
            }
            catch (Exception e) {
            ...
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                msg, "AppStoreServer Search",
                JOptionPane.WARNING_MESSAGE
            );
            if (compFocus != null) compFocus.requestFocus();
        }
        else {
            int max = getModel().getAppStoreServerSearch().getMaxResults();
            if (max > 0 && getHub().getSize() == max) {
                String msg = "Selected the first (max="+max+") App Store Servers.";
                JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                    msg, "AppStoreServer Search",
    ...
    public AppStoreServer getSelected() {
        return appStoreServerSelected;
    }
