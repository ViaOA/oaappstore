...
public class EnvironmentSearchJfc {
    private static Logger LOG = Logger.getLogger(EnvironmentSearchJfc.class.getName());
    
    private EnvironmentSearchModel model;
    private JDialog dialog;
    protected EnvironmentJfc jfcEnvironment;
    
    protected boolean bShowResults;
    protected boolean bMultiSelect;
    protected Environment environmentSelected;
    protected boolean bSelected;
    
    public EnvironmentSearchJfc() {
        this(null, true, false);
    }
    public EnvironmentSearchJfc(EnvironmentSearchModel model) {
        this(model, true, false);
    }
    public EnvironmentSearchJfc(EnvironmentSearchModel model, boolean bShowResults, boolean bMultiSelect) {
        this.model = model;
        this.bShowResults = bShowResults;
        this.bMultiSelect = bMultiSelect;
    }
    public EnvironmentSearchJfc(boolean bShowResults) {
        this(null, bShowResults, false);
    }
    
    public EnvironmentSearchModel getModel() {
        if (model == null) {
            model = new EnvironmentSearchModel();
            model.getEnvironmentSearch().setMaxResults(100); // default for UI searches
        }
        return model;
    }
    ...
    public JPanel createMainPanel() {
        ...
        if (cnt > 1) {
            ...
            Icon icon = Resource.getJarIcon("environment.gif");
            if (panSearch != null) {
    ...
    public JPanel createSearchFromPanel() {
        final Hub<Environment> hub = getModel().getSearchFromHub();
        ...
        if (bMultiSelect) table.setMultiSelectControlKey(true);
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    Environment environment = hub.getAO();
                    if (environment != null) onSelect(environment, hub);
                }
            });
        }
        return pan;
    ...
    public JPanel createMultiSelectPanel() {
        final Hub<Environment> hub = getModel().getMultiSelectHub();
        JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        JLabel lbl = new JLabel("Environments to add");
    ...
    public JPanel createSearchTreePanel() {
        // no tree properties defined in model, for Environment
        if (true || false) return null;
        // if (!bShowResults) return null;
        final JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        final EnvironmentModel model = new EnvironmentModel(ModelDelegate.getEnvironments().createSharedHub());
        if (bMultiSelect) {
        ...
        if (!bMultiSelect) {
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    Environment environment = model.getHub().getAO(); 
                    if (environment != null) onSelect(environment, model.getHub());
                }
            });
            pan.add(panx, BorderLayout.SOUTH);
        }
        ...
        SwingWorker<OATree, Void> sw = new SwingWorker<OATree, Void>() {
            ...
            protected OATree doInBackground() throws Exception {
                EnvironmentJfc jfcEnvironment = new EnvironmentJfc(model);
                OATree tree = jfcEnvironment.createSearchTree();
                tree.preload();
                return tree;
            }
            @Override
    ...
    protected JPanel createSearchResultsPanel() {
        ...
        final Hub<Environment> hub = getHub();
        OATable table = createReadOnlyTable(hub);
        OATableScrollPane spTable = new OATableScrollPane(table, 1);
    ...
    public OATextField createNameTextField() {
        OATextField txt = new OATextField(getModel().getEnvironmentSearchHub(), EnvironmentSearch.P_Name, 15);
    ...
    public OATextField createAbbrevNameTextField() {
        OATextField txt = new OATextField(getModel().getEnvironmentSearchHub(), EnvironmentSearch.P_AbbrevName, 5);
        txt.setMinimumColumns(0);
    ...
    public OAComboBox createTypeComboBox() {
        Hub<String> hub = OAObjectEnumDelegate.getDisplayNameValues(EnvironmentSearch.class, EnvironmentSearch.P_Type).createSharedHub();
        hub.setLinkHubOnPos(getModel().getEnvironmentSearchHub(), EnvironmentSearch.P_Type);
    ...
    public OATextField createMaxResultsTextField() {
        OATextField txt = new OATextField(getModel().getEnvironmentSearchHub(), EnvironmentSearch.P_MaxResults, 5);
    ...
    public OATable createReadOnlyTable(final Hub<Environment> hubEnvironment) {
        Hub<Environment> hubx = new Hub(Environment.class);
        OATable table = new OATable(hubx) {
            @Override
            public void onDoubleClick() {
                if (EnvironmentSearchJfc.this.getModel().getMultiSelectHub() != hubEnvironment) {
                    Environment environment = hubEnvironment.getAO();
                    if (environment != null) onSelect(environment, hubEnvironment);
                }
            }
        };
        table.setFilterMasterHub(hubEnvironment);
        table.setAllowDnD(false);
        table.setAllowSorting(true);
        table.addCounterColumn();
    
        if (bMultiSelect && getModel().getMultiSelectHub() != hubEnvironment) {
    ...
    protected void createTableColumns(OATable table) {
        getEnvironmentJfc().createReadOnlyTableColumns(table);
    }
    public OAButton createSelectOneButton(final Hub<Environment> hub) {
        ...
        cmd.setToolTipText("Add selected Environment");
        OAButton.setup(cmd);
    ...
    public JButton createSelectManyButton() {
        ...
        cmd.setToolTipText("Add selected Environments");
    ...
    public OAButton createRemoveButton(Hub<Environment> hub) {
        OAButton cmd = new OAButton(hub, OAButton.REMOVE);
        cmd.setText("Remove");
        cmd.setup();
        cmd.setToolTipText("Remove selected Environment");
        return cmd;
    }
    public OAButton createRemoveAllButton(Hub<Environment> hub) {
        ...
        cmd.setup();
        cmd.setToolTipText("Remove all Environments from the select list");
        return cmd;
    }
    ...
    public EnvironmentJfc getEnvironmentJfc() {
        if (jfcEnvironment == null) {
            jfcEnvironment = new EnvironmentJfc(new Hub<Environment>(Environment.class));
            OAObjectModel om = jfcEnvironment.getModel();
    ...
    public OAButton createGotoEditButton(final Hub<Environment> hub) {
        OAButton cmd = new OAButton(hub, "Details ...", Resource.getJarIcon(Resource.IMG_Goto), OAButton.GOTO) {
            @Override
            public void afterActionPerformed() {
                getEnvironmentJfc().getHub().setSharedHub(hub, true);
                getEnvironmentJfc().getEditDialog(this).setVisible(true);
    ...
    public JDialog getDialog() {
        ...
        dialog = new JDialog(win, "Environment Search", ModalityType.APPLICATION_MODAL);
        ...
        dialog.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                EnvironmentSearchJfc.this.bSelected = false;
                onShowDialog();
            }
            @Override
            public void windowClosing(WindowEvent e) {
                if (!EnvironmentSearchJfc.this.bSelected) {
                    EnvironmentSearchJfc.this.onCancel();
                }
                onHideDialog();
            }
        });
    ...
    protected void onReset() {
        getHub().cancelSelect();
        getHub().clear();
        getModel().getEnvironmentSearch().reset();
    }
    
    protected void onSelect(Environment environment, Hub<Environment> hubFrom) {
        environmentSelected = environment;
        bSelected = true;
        getHub().cancelSelect();
        getModel().onSelect(environment, hubFrom);
        if (dialog != null) dialog.setVisible(false);
    }
    protected void onSelect(Hub<Environment> hub) {
        bSelected = true;
        getHub().cancelSelect();
    ...
    protected void onSearch(final JComponent compFrom) {
        /*
        if (!getModel().getEnvironmentSearch().isDataEntered()) {
            String msg = "Please enter selection data.";
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom), msg, "Environment Search", JOptionPane.WARNING_MESSAGE);
            return;
        }
        */
        ...
        dlgWait.setStatus("Selecting Environments ... please wait ...");
        dlgWait.setTitle("Environment Search");
        ...
        if (dlgWait.wasCancelled()) {
            ...
            String msg = "No Environments were found.";
            try {
                sw.get();
            }
            catch (Exception e) {
            ...
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                msg, "Environment Search",
                JOptionPane.WARNING_MESSAGE
            );
            if (compFocus != null) compFocus.requestFocus();
        }
        else {
            int max = getModel().getEnvironmentSearch().getMaxResults();
            if (max > 0 && getHub().getSize() == max) {
                String msg = "Selected the first (max="+max+") Environments.";
                JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                    msg, "Environment Search",
