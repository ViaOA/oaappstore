...
public class ApplicationTypeSearchJfc {
    private static Logger LOG = Logger.getLogger(ApplicationTypeSearchJfc.class.getName());
    
    private ApplicationTypeSearchModel model;
    private JDialog dialog;
    protected ApplicationTypeJfc jfcApplicationType;
    
    protected boolean bShowResults;
    protected boolean bMultiSelect;
    protected ApplicationType applicationTypeSelected;
    protected boolean bSelected;
    
    public ApplicationTypeSearchJfc() {
        this(null, true, false);
    }
    public ApplicationTypeSearchJfc(ApplicationTypeSearchModel model) {
        this(model, true, false);
    }
    public ApplicationTypeSearchJfc(ApplicationTypeSearchModel model, boolean bShowResults, boolean bMultiSelect) {
        this.model = model;
        this.bShowResults = bShowResults;
        this.bMultiSelect = bMultiSelect;
    }
    public ApplicationTypeSearchJfc(boolean bShowResults) {
        this(null, bShowResults, false);
    }
    
    public ApplicationTypeSearchModel getModel() {
        if (model == null) {
            model = new ApplicationTypeSearchModel();
            model.getApplicationTypeSearch().setMaxResults(100); // default for UI searches
        }
        return model;
    }
    ...
    public JPanel createMainPanel() {
        ...
        if (cnt > 1) {
            ...
            Icon icon = Resource.getJarIcon("applicationType.gif");
            if (panSearch != null) {
    ...
    public JPanel createSearchFromPanel() {
        final Hub<ApplicationType> hub = getModel().getSearchFromHub();
        ...
        if (bMultiSelect) table.setMultiSelectControlKey(true);
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    ApplicationType applicationType = hub.getAO();
                    if (applicationType != null) onSelect(applicationType, hub);
                }
            });
        }
        return pan;
    ...
    public JPanel createMultiSelectPanel() {
        final Hub<ApplicationType> hub = getModel().getMultiSelectHub();
        JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        JLabel lbl = new JLabel("ApplicationTypes to add");
    ...
    public JPanel createSearchTreePanel() {
        // no tree properties defined in model, for ApplicationType
        if (true || false) return null;
        // if (!bShowResults) return null;
        final JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        final ApplicationTypeModel model = new ApplicationTypeModel(ModelDelegate.getApplicationTypes().createSharedHub());
        if (bMultiSelect) {
        ...
        if (!bMultiSelect) {
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    ApplicationType applicationType = model.getHub().getAO(); 
                    if (applicationType != null) onSelect(applicationType, model.getHub());
                }
            });
            pan.add(panx, BorderLayout.SOUTH);
        }
        ...
        SwingWorker<OATree, Void> sw = new SwingWorker<OATree, Void>() {
            ...
            protected OATree doInBackground() throws Exception {
                ApplicationTypeJfc jfcApplicationType = new ApplicationTypeJfc(model);
                OATree tree = jfcApplicationType.createSearchTree();
                tree.preload();
                return tree;
            }
            @Override
    ...
    protected JPanel createSearchResultsPanel() {
        ...
        final Hub<ApplicationType> hub = getHub();
        OATable table = createReadOnlyTable(hub);
        OATableScrollPane spTable = new OATableScrollPane(table, 1);
    ...
    public OATextField createAbbrevNameTextField() {
        OATextField txt = new OATextField(getModel().getApplicationTypeSearchHub(), ApplicationTypeSearch.P_AbbrevName, 8);
        txt.setMinimumColumns(0);
        txt.setMaximumColumns(8);
        // setup(txt);
        return txt;
    ...
    public OATextField createClientPortTextField() {
        OATextField txt = new OATextField(getModel().getApplicationTypeSearchHub(), ApplicationTypeSearch.P_ClientPort, 4);
        txt.setMinimumColumns(0);
        txt.setMaximumColumns(8);
        txt.setFormat("#");
        // setup(txt);
    ...
    public OATextField createHttpsPortTextField() {
        OATextField txt = new OATextField(getModel().getApplicationTypeSearchHub(), ApplicationTypeSearch.P_HttpsPort, 4);
        txt.setMinimumColumns(0);
    ...
    public OATextField createMaxResultsTextField() {
        OATextField txt = new OATextField(getModel().getApplicationTypeSearchHub(), ApplicationTypeSearch.P_MaxResults, 5);
    ...
    public OATable createReadOnlyTable(final Hub<ApplicationType> hubApplicationType) {
        Hub<ApplicationType> hubx = new Hub(ApplicationType.class);
        OATable table = new OATable(hubx) {
            @Override
            public void onDoubleClick() {
                if (ApplicationTypeSearchJfc.this.getModel().getMultiSelectHub() != hubApplicationType) {
                    ApplicationType applicationType = hubApplicationType.getAO();
                    if (applicationType != null) onSelect(applicationType, hubApplicationType);
                }
            }
        };
        table.setFilterMasterHub(hubApplicationType);
        table.setAllowDnD(false);
        table.setAllowSorting(true);
        table.addCounterColumn();
    
        if (bMultiSelect && getModel().getMultiSelectHub() != hubApplicationType) {
    ...
    protected void createTableColumns(OATable table) {
        getApplicationTypeJfc().createReadOnlyTableColumns(table);
    }
    public OAButton createSelectOneButton(final Hub<ApplicationType> hub) {
        ...
        cmd.setToolTipText("Add selected ApplicationType");
        OAButton.setup(cmd);
    ...
    public JButton createSelectManyButton() {
        ...
        cmd.setToolTipText("Add selected ApplicationTypes");
    ...
    public OAButton createRemoveButton(Hub<ApplicationType> hub) {
        OAButton cmd = new OAButton(hub, OAButton.REMOVE);
        cmd.setText("Remove");
        cmd.setup();
        cmd.setToolTipText("Remove selected ApplicationType");
        return cmd;
    }
    public OAButton createRemoveAllButton(Hub<ApplicationType> hub) {
        ...
        cmd.setup();
        cmd.setToolTipText("Remove all ApplicationTypes from the select list");
        return cmd;
    }
    ...
    public ApplicationTypeJfc getApplicationTypeJfc() {
        if (jfcApplicationType == null) {
            jfcApplicationType = new ApplicationTypeJfc(new Hub<ApplicationType>(ApplicationType.class));
            OAObjectModel om = jfcApplicationType.getModel();
    ...
    public OAButton createGotoEditButton(final Hub<ApplicationType> hub) {
        OAButton cmd = new OAButton(hub, "Details ...", Resource.getJarIcon(Resource.IMG_Goto), OAButton.GOTO) {
            @Override
            public void afterActionPerformed() {
                getApplicationTypeJfc().getHub().setSharedHub(hub, true);
                getApplicationTypeJfc().getEditDialog(this).setVisible(true);
    ...
    public JDialog getDialog() {
        ...
        dialog = new JDialog(win, "Application Type Search", ModalityType.APPLICATION_MODAL);
        ...
        dialog.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                ApplicationTypeSearchJfc.this.bSelected = false;
                onShowDialog();
            }
            @Override
            public void windowClosing(WindowEvent e) {
                if (!ApplicationTypeSearchJfc.this.bSelected) {
                    ApplicationTypeSearchJfc.this.onCancel();
                }
                onHideDialog();
            }
        });
    ...
    protected void onReset() {
        getHub().cancelSelect();
        getHub().clear();
        getModel().getApplicationTypeSearch().reset();
    }
    
    protected void onSelect(ApplicationType applicationType, Hub<ApplicationType> hubFrom) {
        applicationTypeSelected = applicationType;
        bSelected = true;
        getHub().cancelSelect();
        getModel().onSelect(applicationType, hubFrom);
    ...
    protected void onSearch(final JComponent compFrom) {
        /*
        if (!getModel().getApplicationTypeSearch().isDataEntered()) {
            String msg = "Please enter selection data.";
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom), msg, "ApplicationType Search", JOptionPane.WARNING_MESSAGE);
            return;
        }
        */
        ...
        dlgWait.setStatus("Selecting ApplicationTypes ... please wait ...");
        dlgWait.setTitle("ApplicationType Search");
        ...
        if (dlgWait.wasCancelled()) {
            ...
            String msg = "No ApplicationTypes were found.";
            try {
                sw.get();
            }
            catch (Exception e) {
            ...
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                msg, "ApplicationType Search",
                JOptionPane.WARNING_MESSAGE
            );
            if (compFocus != null) compFocus.requestFocus();
        }
        else {
            int max = getModel().getApplicationTypeSearch().getMaxResults();
            if (max > 0 && getHub().getSize() == max) {
                String msg = "Selected the first (max="+max+") Application Types.";
                JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                    msg, "ApplicationType Search",
    ...
    public ApplicationType getSelected() {
        return applicationTypeSelected;
    }
