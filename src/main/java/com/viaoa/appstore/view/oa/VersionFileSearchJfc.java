...
public class VersionFileSearchJfc {
    private static Logger LOG = Logger.getLogger(VersionFileSearchJfc.class.getName());
    
    private VersionFileSearchModel model;
    private JDialog dialog;
    protected VersionFileJfc jfcVersionFile;
    
    protected boolean bShowResults;
    protected boolean bMultiSelect;
    protected VersionFile versionFileSelected;
    protected boolean bSelected;
    
    public VersionFileSearchJfc() {
        this(null, true, false);
    }
    public VersionFileSearchJfc(VersionFileSearchModel model) {
        this(model, true, false);
    }
    public VersionFileSearchJfc(VersionFileSearchModel model, boolean bShowResults, boolean bMultiSelect) {
        this.model = model;
        this.bShowResults = bShowResults;
        this.bMultiSelect = bMultiSelect;
    }
    public VersionFileSearchJfc(boolean bShowResults) {
        this(null, bShowResults, false);
    }
    
    public VersionFileSearchModel getModel() {
        if (model == null) {
            model = new VersionFileSearchModel();
            model.getVersionFileSearch().setMaxResults(100); // default for UI searches
        }
        return model;
    }
    ...
    public JPanel createMainPanel() {
        ...
        if (cnt > 1) {
            ...
            Icon icon = Resource.getJarIcon("versionFile.gif");
            if (panSearch != null) {
    ...
    public JPanel createSearchFromPanel() {
        final Hub<VersionFile> hub = getModel().getSearchFromHub();
        ...
        if (bMultiSelect) table.setMultiSelectControlKey(true);
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    VersionFile versionFile = hub.getAO();
                    if (versionFile != null) onSelect(versionFile, hub);
                }
            });
        }
        return pan;
    ...
    public JPanel createMultiSelectPanel() {
        final Hub<VersionFile> hub = getModel().getMultiSelectHub();
        JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        JLabel lbl = new JLabel("VersionFiles to add");
    ...
    public JPanel createSearchTreePanel() {
        // no tree properties defined in model, for VersionFile
        if (true || false) return null;
        // if (!bShowResults) return null;
        final JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        final VersionFileModel model = new VersionFileModel();
        if (bMultiSelect) {
        ...
        if (!bMultiSelect) {
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    VersionFile versionFile = model.getHub().getAO(); 
                    if (versionFile != null) onSelect(versionFile, model.getHub());
                }
            });
            pan.add(panx, BorderLayout.SOUTH);
        }
        ...
        SwingWorker<OATree, Void> sw = new SwingWorker<OATree, Void>() {
            ...
            protected OATree doInBackground() throws Exception {
                VersionFileJfc jfcVersionFile = new VersionFileJfc(model);
                OATree tree = jfcVersionFile.createSearchTree();
                tree.preload();
                return tree;
            }
            @Override
    ...
    protected JPanel createSearchResultsPanel() {
        ...
        final Hub<VersionFile> hub = getHub();
        OATable table = createReadOnlyTable(hub);
        OATableScrollPane spTable = new OATableScrollPane(table, 1);
    ...
    public OATextField createMaxResultsTextField() {
        OATextField txt = new OATextField(getModel().getVersionFileSearchHub(), VersionFileSearch.P_MaxResults, 5);
    ...
    public OATable createReadOnlyTable(final Hub<VersionFile> hubVersionFile) {
        Hub<VersionFile> hubx = new Hub(VersionFile.class);
        OATable table = new OATable(hubx) {
            @Override
            public void onDoubleClick() {
                if (VersionFileSearchJfc.this.getModel().getMultiSelectHub() != hubVersionFile) {
                    VersionFile versionFile = hubVersionFile.getAO();
                    if (versionFile != null) onSelect(versionFile, hubVersionFile);
                }
            }
        };
        table.setFilterMasterHub(hubVersionFile);
        table.setAllowDnD(false);
        table.setAllowSorting(true);
        table.addCounterColumn();
    
        if (bMultiSelect && getModel().getMultiSelectHub() != hubVersionFile) {
    ...
    protected void createTableColumns(OATable table) {
        getVersionFileJfc().createReadOnlyTableColumns(table);
    }
    public OAButton createSelectOneButton(final Hub<VersionFile> hub) {
        ...
        cmd.setToolTipText("Add selected VersionFile");
        OAButton.setup(cmd);
    ...
    public JButton createSelectManyButton() {
        ...
        cmd.setToolTipText("Add selected VersionFiles");
    ...
    public OAButton createRemoveButton(Hub<VersionFile> hub) {
        OAButton cmd = new OAButton(hub, OAButton.REMOVE);
        cmd.setText("Remove");
        cmd.setup();
        cmd.setToolTipText("Remove selected VersionFile");
        return cmd;
    }
    public OAButton createRemoveAllButton(Hub<VersionFile> hub) {
        ...
        cmd.setup();
        cmd.setToolTipText("Remove all VersionFiles from the select list");
        return cmd;
    }
    ...
    public VersionFileJfc getVersionFileJfc() {
        if (jfcVersionFile == null) {
            jfcVersionFile = new VersionFileJfc(new Hub<VersionFile>(VersionFile.class));
            OAObjectModel om = jfcVersionFile.getModel();
        ...
        return jfcVersionFile;
    }
    public OAButton createGotoEditButton(final Hub<VersionFile> hub) {
        OAButton cmd = new OAButton(hub, "Details ...", Resource.getJarIcon(Resource.IMG_Goto), OAButton.GOTO) {
            @Override
            public void afterActionPerformed() {
                getVersionFileJfc().getHub().setSharedHub(hub, true);
                getVersionFileJfc().getEditDialog(this).setVisible(true);
    ...
    public JDialog getDialog() {
        ...
        dialog = new JDialog(win, "Version File Search", ModalityType.APPLICATION_MODAL);
        ...
        dialog.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                VersionFileSearchJfc.this.bSelected = false;
                onShowDialog();
            }
            @Override
            public void windowClosing(WindowEvent e) {
                if (!VersionFileSearchJfc.this.bSelected) {
                    VersionFileSearchJfc.this.onCancel();
                }
                onHideDialog();
            }
        });
    ...
    protected void onReset() {
        getHub().cancelSelect();
        getHub().clear();
        getModel().getVersionFileSearch().reset();
    }
    
    protected void onSelect(VersionFile versionFile, Hub<VersionFile> hubFrom) {
        versionFileSelected = versionFile;
        bSelected = true;
        getHub().cancelSelect();
        getModel().onSelect(versionFile, hubFrom);
        if (dialog != null) dialog.setVisible(false);
    }
    protected void onSelect(Hub<VersionFile> hub) {
        bSelected = true;
        getHub().cancelSelect();
    ...
    protected void onSearch(final JComponent compFrom) {
        /*
        if (!getModel().getVersionFileSearch().isDataEntered()) {
            String msg = "Please enter selection data.";
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom), msg, "VersionFile Search", JOptionPane.WARNING_MESSAGE);
            return;
        }
        */
        ...
        dlgWait.setStatus("Selecting VersionFiles ... please wait ...");
        dlgWait.setTitle("VersionFile Search");
        ...
        if (dlgWait.wasCancelled()) {
            ...
            String msg = "No VersionFiles were found.";
            try {
                sw.get();
            }
            catch (Exception e) {
            ...
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                msg, "VersionFile Search",
                JOptionPane.WARNING_MESSAGE
            );
            if (compFocus != null) compFocus.requestFocus();
        }
        else {
            int max = getModel().getVersionFileSearch().getMaxResults();
            if (max > 0 && getHub().getSize() == max) {
                String msg = "Selected the first (max="+max+") Version Files.";
                JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                    msg, "VersionFile Search",
    ...
    public VersionFile getSelected() {
        return versionFileSelected;
    }
