...
public class PropertyValueSearchJfc {
    private static Logger LOG = Logger.getLogger(PropertyValueSearchJfc.class.getName());
    
    private PropertyValueSearchModel model;
    private JDialog dialog;
    protected PropertyValueJfc jfcPropertyValue;
    
    protected boolean bShowResults;
    protected boolean bMultiSelect;
    protected PropertyValue propertyValueSelected;
    protected boolean bSelected;
    
    public PropertyValueSearchJfc() {
        this(null, true, false);
    }
    public PropertyValueSearchJfc(PropertyValueSearchModel model) {
        this(model, true, false);
    }
    public PropertyValueSearchJfc(PropertyValueSearchModel model, boolean bShowResults, boolean bMultiSelect) {
        this.model = model;
        this.bShowResults = bShowResults;
        this.bMultiSelect = bMultiSelect;
    }
    public PropertyValueSearchJfc(boolean bShowResults) {
        this(null, bShowResults, false);
    }
    
    public PropertyValueSearchModel getModel() {
        if (model == null) {
            model = new PropertyValueSearchModel();
            model.getPropertyValueSearch().setMaxResults(100); // default for UI searches
        }
        return model;
    }
    ...
    public JPanel createMainPanel() {
        ...
        if (cnt > 1) {
            ...
            Icon icon = Resource.getJarIcon("propertyValue.gif");
            if (panSearch != null) {
    ...
    public JPanel createSearchFromPanel() {
        final Hub<PropertyValue> hub = getModel().getSearchFromHub();
        ...
        if (bMultiSelect) table.setMultiSelectControlKey(true);
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    PropertyValue propertyValue = hub.getAO();
                    if (propertyValue != null) onSelect(propertyValue, hub);
                }
            });
        }
        return pan;
    ...
    public JPanel createMultiSelectPanel() {
        final Hub<PropertyValue> hub = getModel().getMultiSelectHub();
        JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        JLabel lbl = new JLabel("PropertyValues to add");
    ...
    public JPanel createSearchTreePanel() {
        // no tree properties defined in model, for PropertyValue
        if (true || false) return null;
        // if (!bShowResults) return null;
        final JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        final PropertyValueModel model = new PropertyValueModel();
        if (bMultiSelect) {
        ...
        if (!bMultiSelect) {
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    PropertyValue propertyValue = model.getHub().getAO(); 
                    if (propertyValue != null) onSelect(propertyValue, model.getHub());
                }
            });
            pan.add(panx, BorderLayout.SOUTH);
        }
        ...
        SwingWorker<OATree, Void> sw = new SwingWorker<OATree, Void>() {
            ...
            protected OATree doInBackground() throws Exception {
                PropertyValueJfc jfcPropertyValue = new PropertyValueJfc(model);
                OATree tree = jfcPropertyValue.createSearchTree();
                tree.preload();
                return tree;
            }
            @Override
    ...
    protected JPanel createSearchResultsPanel() {
        ...
        final Hub<PropertyValue> hub = getHub();
        OATable table = createReadOnlyTable(hub);
        OATableScrollPane spTable = new OATableScrollPane(table, 1);
    ...
    public OATextField createMaxResultsTextField() {
        OATextField txt = new OATextField(getModel().getPropertyValueSearchHub(), PropertyValueSearch.P_MaxResults, 5);
    ...
    public OATable createReadOnlyTable(final Hub<PropertyValue> hubPropertyValue) {
        Hub<PropertyValue> hubx = new Hub(PropertyValue.class);
        OATable table = new OATable(hubx) {
            @Override
            public void onDoubleClick() {
                if (PropertyValueSearchJfc.this.getModel().getMultiSelectHub() != hubPropertyValue) {
                    PropertyValue propertyValue = hubPropertyValue.getAO();
                    if (propertyValue != null) onSelect(propertyValue, hubPropertyValue);
                }
            }
        };
        table.setFilterMasterHub(hubPropertyValue);
        table.setAllowDnD(false);
        table.setAllowSorting(true);
        table.addCounterColumn();
    
        if (bMultiSelect && getModel().getMultiSelectHub() != hubPropertyValue) {
    ...
    protected void createTableColumns(OATable table) {
        getPropertyValueJfc().createReadOnlyTableColumns(table);
    }
    public OAButton createSelectOneButton(final Hub<PropertyValue> hub) {
        ...
        cmd.setToolTipText("Add selected PropertyValue");
        OAButton.setup(cmd);
    ...
    public JButton createSelectManyButton() {
        ...
        cmd.setToolTipText("Add selected PropertyValues");
    ...
    public OAButton createRemoveButton(Hub<PropertyValue> hub) {
        OAButton cmd = new OAButton(hub, OAButton.REMOVE);
        cmd.setText("Remove");
        cmd.setup();
        cmd.setToolTipText("Remove selected PropertyValue");
        return cmd;
    }
    public OAButton createRemoveAllButton(Hub<PropertyValue> hub) {
        ...
        cmd.setup();
        cmd.setToolTipText("Remove all PropertyValues from the select list");
        return cmd;
    }
    ...
    public PropertyValueJfc getPropertyValueJfc() {
        if (jfcPropertyValue == null) {
            jfcPropertyValue = new PropertyValueJfc(new Hub<PropertyValue>(PropertyValue.class));
            OAObjectModel om = jfcPropertyValue.getModel();
        ...
        return jfcPropertyValue;
    }
    public OAButton createGotoEditButton(final Hub<PropertyValue> hub) {
        OAButton cmd = new OAButton(hub, "Details ...", Resource.getJarIcon(Resource.IMG_Goto), OAButton.GOTO) {
            @Override
            public void afterActionPerformed() {
                getPropertyValueJfc().getHub().setSharedHub(hub, true);
                getPropertyValueJfc().getEditDialog(this).setVisible(true);
    ...
    public JDialog getDialog() {
        ...
        dialog = new JDialog(win, "Property Value Search", ModalityType.APPLICATION_MODAL);
        ...
        dialog.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                PropertyValueSearchJfc.this.bSelected = false;
                onShowDialog();
            }
            @Override
            public void windowClosing(WindowEvent e) {
                if (!PropertyValueSearchJfc.this.bSelected) {
                    PropertyValueSearchJfc.this.onCancel();
                }
                onHideDialog();
            }
        });
    ...
    protected void onReset() {
        getHub().cancelSelect();
        getHub().clear();
        getModel().getPropertyValueSearch().reset();
    }
    
    protected void onSelect(PropertyValue propertyValue, Hub<PropertyValue> hubFrom) {
        propertyValueSelected = propertyValue;
        bSelected = true;
        getHub().cancelSelect();
        getModel().onSelect(propertyValue, hubFrom);
        if (dialog != null) dialog.setVisible(false);
    }
    protected void onSelect(Hub<PropertyValue> hub) {
        bSelected = true;
        getHub().cancelSelect();
    ...
    protected void onSearch(final JComponent compFrom) {
        /*
        if (!getModel().getPropertyValueSearch().isDataEntered()) {
            String msg = "Please enter selection data.";
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom), msg, "PropertyValue Search", JOptionPane.WARNING_MESSAGE);
            return;
        }
        */
        ...
        dlgWait.setStatus("Selecting PropertyValues ... please wait ...");
        dlgWait.setTitle("PropertyValue Search");
        ...
        if (dlgWait.wasCancelled()) {
            ...
            String msg = "No PropertyValues were found.";
            try {
                sw.get();
            }
            catch (Exception e) {
            ...
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                msg, "PropertyValue Search",
                JOptionPane.WARNING_MESSAGE
            );
            if (compFocus != null) compFocus.requestFocus();
        }
        else {
            int max = getModel().getPropertyValueSearch().getMaxResults();
            if (max > 0 && getHub().getSize() == max) {
                String msg = "Selected the first (max="+max+") Property Values.";
                JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                    msg, "PropertyValue Search",
