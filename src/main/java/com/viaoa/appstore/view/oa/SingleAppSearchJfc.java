...
public class SingleAppSearchJfc {
    private static Logger LOG = Logger.getLogger(SingleAppSearchJfc.class.getName());
    
    private SingleAppSearchModel model;
    private JDialog dialog;
    protected SingleAppJfc jfcSingleApp;
    protected ApplicationTypeJfc jfcApplicationType;
    
    protected boolean bShowResults;
    protected boolean bMultiSelect;
    protected SingleApp singleAppSelected;
    protected boolean bSelected;
    
    public SingleAppSearchJfc() {
        this(null, true, false);
    }
    public SingleAppSearchJfc(SingleAppSearchModel model) {
        this(model, true, false);
    }
    public SingleAppSearchJfc(SingleAppSearchModel model, boolean bShowResults, boolean bMultiSelect) {
        this.model = model;
        this.bShowResults = bShowResults;
        this.bMultiSelect = bMultiSelect;
    }
    public SingleAppSearchJfc(boolean bShowResults) {
        this(null, bShowResults, false);
    }
    
    public SingleAppSearchModel getModel() {
        if (model == null) {
            model = new SingleAppSearchModel();
            model.getSingleAppSearch().setMaxResults(100); // default for UI searches
        }
        return model;
    }
    ...
    public JPanel createMainPanel() {
        ...
        if (cnt > 1) {
            ...
            Icon icon = Resource.getJarIcon("singleApp.gif");
            if (panSearch != null) {
    ...
    public JPanel createSearchFromPanel() {
        final Hub<SingleApp> hub = getModel().getSearchFromHub();
        ...
        if (bMultiSelect) table.setMultiSelectControlKey(true);
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    SingleApp singleApp = hub.getAO();
                    if (singleApp != null) onSelect(singleApp, hub);
                }
            });
        }
        return pan;
    ...
    public JPanel createMultiSelectPanel() {
        final Hub<SingleApp> hub = getModel().getMultiSelectHub();
        JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        JLabel lbl = new JLabel("SingleApps to add");
    ...
    public JPanel createSearchTreePanel() {
        // no tree properties defined in model, for SingleApp
        if (true || false) return null;
        // if (!bShowResults) return null;
        final JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        final SingleAppModel model = new SingleAppModel();
        if (bMultiSelect) {
        ...
        if (!bMultiSelect) {
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    SingleApp singleApp = model.getHub().getAO(); 
                    if (singleApp != null) onSelect(singleApp, model.getHub());
                }
            });
            pan.add(panx, BorderLayout.SOUTH);
        }
        ...
        SwingWorker<OATree, Void> sw = new SwingWorker<OATree, Void>() {
            ...
            protected OATree doInBackground() throws Exception {
                SingleAppJfc jfcSingleApp = new SingleAppJfc(model);
                OATree tree = jfcSingleApp.createSearchTree();
                tree.preload();
                return tree;
            }
            @Override
    ...
    protected JPanel createSearchInputPanel() {
        ...
        cboTable.setLabel(lbl, getModel().getSingleAppSearchHub());
        panel.add(lbl, gc);
    ...
    protected JPanel createSearchResultsPanel() {
        ...
        final Hub<SingleApp> hub = getHub();
        OATable table = createReadOnlyTable(hub);
        OATableScrollPane spTable = new OATableScrollPane(table, 1);
    ...
    public OATextField createIdTextField() {
        OATextField txt = new OATextField(getModel().getSingleAppSearchHub(), SingleAppSearch.P_Id, 6);
    ...
    public OADateComboBox createCreatedDateComboBox() {
        OADateComboBox dcbo = new OADateComboBox(getModel().getSingleAppSearchHub(), SingleAppSearch.P_Created, 15);
        dcbo.setMaximumColumns(22);
        OATextField txt = new OATextField(getModel().getSingleAppSearchHub(), SingleAppSearch.P_Created, 10);
        dcbo.setEditor(txt);
        return dcbo;
    }
    public OADateComboBox createCreated2DateComboBox() {
        OADateComboBox dcbo = new OADateComboBox(getModel().getSingleAppSearchHub(), SingleAppSearch.P_Created2, 15);
        dcbo.setMaximumColumns(22);
        OATextField txt = new OATextField(getModel().getSingleAppSearchHub(), SingleAppSearch.P_Created2, 10);
        dcbo.setEditor(txt);
        return dcbo;
    }
    
    public OATextField createMaxResultsTextField() {
        OATextField txt = new OATextField(getModel().getSingleAppSearchHub(), SingleAppSearch.P_MaxResults, 5);
    ...
    public JButton createApplicationTypeCommand() {
        ...
        if (h != null && h.getObjectClass().equals(AppUser.class)) {
            b = false;
        }
        if (b) {
            cmd = new OAButton(getModel().getSingleAppSearchHub());
            ((OAButton) cmd).setUpdateObject(SingleAppSearch.P_ApplicationType, null);
    ...
    public OACheckBox createUseApplicationTypeSearchCheckBox() {
        OACheckBox chk = new OACheckBox(getModel().getSingleAppSearchHub(), SingleAppSearch.P_UseApplicationTypeSearch);
    ...
    public OATable createReadOnlyTable(final Hub<SingleApp> hubSingleApp) {
        Hub<SingleApp> hubx = new Hub(SingleApp.class);
        OATable table = new OATable(hubx) {
            @Override
            public void onDoubleClick() {
                if (SingleAppSearchJfc.this.getModel().getMultiSelectHub() != hubSingleApp) {
                    SingleApp singleApp = hubSingleApp.getAO();
                    if (singleApp != null) onSelect(singleApp, hubSingleApp);
                }
            }
        };
        table.setFilterMasterHub(hubSingleApp);
        table.setAllowDnD(false);
        table.setAllowSorting(true);
        table.addCounterColumn();
    
        if (bMultiSelect && getModel().getMultiSelectHub() != hubSingleApp) {
    ...
    protected void createTableColumns(OATable table) {
        getSingleAppJfc().createReadOnlyTableColumns(table);
    }
    public OAButton createSelectOneButton(final Hub<SingleApp> hub) {
        ...
        cmd.setToolTipText("Add selected SingleApp");
        OAButton.setup(cmd);
    ...
    public JButton createSelectManyButton() {
        ...
        cmd.setToolTipText("Add selected SingleApps");
    ...
    public OAButton createRemoveButton(Hub<SingleApp> hub) {
        OAButton cmd = new OAButton(hub, OAButton.REMOVE);
        cmd.setText("Remove");
        cmd.setup();
        cmd.setToolTipText("Remove selected SingleApp");
        return cmd;
    }
    public OAButton createRemoveAllButton(Hub<SingleApp> hub) {
        ...
        cmd.setup();
        cmd.setToolTipText("Remove all SingleApps from the select list");
        return cmd;
    }
    ...
    public SingleAppJfc getSingleAppJfc() {
        if (jfcSingleApp == null) {
            jfcSingleApp = new SingleAppJfc(new Hub<SingleApp>(SingleApp.class));
            OAObjectModel om = jfcSingleApp.getModel();
    ...
    public OAButton createGotoEditButton(final Hub<SingleApp> hub) {
        OAButton cmd = new OAButton(hub, "Details ...", Resource.getJarIcon(Resource.IMG_Goto), OAButton.GOTO) {
            @Override
            public void afterActionPerformed() {
                getSingleAppJfc().getHub().setSharedHub(hub, true);
                getSingleAppJfc().getEditDialog(this).setVisible(true);
    ...
    public JDialog getDialog() {
        ...
        dialog = new JDialog(win, "Single App Search", ModalityType.APPLICATION_MODAL);
        ...
        dialog.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                SingleAppSearchJfc.this.bSelected = false;
                onShowDialog();
            }
            @Override
            public void windowClosing(WindowEvent e) {
                if (!SingleAppSearchJfc.this.bSelected) {
                    SingleAppSearchJfc.this.onCancel();
                }
                onHideDialog();
            }
        });
    ...
    protected void onReset() {
        getHub().cancelSelect();
        getHub().clear();
        getModel().getSingleAppSearch().reset();
    }
    
    protected void onSelect(SingleApp singleApp, Hub<SingleApp> hubFrom) {
        singleAppSelected = singleApp;
        bSelected = true;
        getHub().cancelSelect();
        getModel().onSelect(singleApp, hubFrom);
        if (dialog != null) dialog.setVisible(false);
    }
    protected void onSelect(Hub<SingleApp> hub) {
        bSelected = true;
        getHub().cancelSelect();
    ...
    protected void onSearch(final JComponent compFrom) {
        /*
        if (!getModel().getSingleAppSearch().isDataEntered()) {
            String msg = "Please enter selection data.";
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom), msg, "SingleApp Search", JOptionPane.WARNING_MESSAGE);
            return;
        }
        */
        ...
        dlgWait.setStatus("Selecting SingleApps ... please wait ...");
        dlgWait.setTitle("SingleApp Search");
        ...
        if (dlgWait.wasCancelled()) {
            ...
            String msg = "No SingleApps were found.";
            try {
                sw.get();
            }
            catch (Exception e) {
            ...
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                msg, "SingleApp Search",
                JOptionPane.WARNING_MESSAGE
            );
            if (compFocus != null) compFocus.requestFocus();
        }
        else {
            int max = getModel().getSingleAppSearch().getMaxResults();
            if (max > 0 && getHub().getSize() == max) {
                String msg = "Selected the first (max="+max+") Single Apps.";
                JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                    msg, "SingleApp Search",
    ...
    public ApplicationTypeJfc getApplicationTypeJfc() {
        ...
        jfcApplicationType = new ApplicationTypeJfc(getModel().getApplicationTypeModel()) {
            ...
            protected ApplicationTypeSearchJfc getSearchJfc() {
                ...
                SingleAppSearchJfc.this.getModel().getApplicationTypeSearchModel().getApplicationTypeSearch().setMaxResults(1000);
                jfcSearch = new ApplicationTypeSearchJfc(SingleAppSearchJfc.this.getModel().getApplicationTypeSearchModel());
                return jfcSearch;
            }
        };
    ...
    public SingleApp getSelected() {
        return singleAppSelected;
    }
