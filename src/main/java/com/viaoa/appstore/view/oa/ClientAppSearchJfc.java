...
public class ClientAppSearchJfc {
    private static Logger LOG = Logger.getLogger(ClientAppSearchJfc.class.getName());
    ...
    protected ClientApp clientAppSelected;
    protected boolean bSelected;
    
    public ClientAppSearchJfc() {
        this(null, true, false);
    }
    public ClientAppSearchJfc(ClientAppSearchModel model) {
        this(model, true, false);
    }
    public ClientAppSearchJfc(ClientAppSearchModel model, boolean bShowResults, boolean bMultiSelect) {
        this.model = model;
        this.bShowResults = bShowResults;
        this.bMultiSelect = bMultiSelect;
    }
    public ClientAppSearchJfc(boolean bShowResults) {
        this(null, bShowResults, false);
    }
    
    public ClientAppSearchModel getModel() {
        if (model == null) {
            model = new ClientAppSearchModel();
            model.getClientAppSearch().setMaxResults(100); // default for UI searches
        }
        return model;
    }
    ...
    public JPanel createMainPanel() {
        ...
        if (cnt > 1) {
            ...
            Icon icon = Resource.getJarIcon("clientApp.gif");
            if (panSearch != null) {
    ...
    public JPanel createSearchFromPanel() {
        final Hub<ClientApp> hub = getModel().getSearchFromHub();
        ...
        if (bMultiSelect) table.setMultiSelectControlKey(true);
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    ClientApp clientApp = hub.getAO();
                    if (clientApp != null) onSelect(clientApp, hub);
                }
            });
        }
        return pan;
    ...
    public JPanel createMultiSelectPanel() {
        final Hub<ClientApp> hub = getModel().getMultiSelectHub();
        JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        JLabel lbl = new JLabel("ClientApps to add");
    ...
    public JPanel createSearchTreePanel() {
        // no tree properties defined in model, for ClientApp
        if (true || false) return null;
        // if (!bShowResults) return null;
        final JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        final ClientAppModel model = new ClientAppModel();
        if (bMultiSelect) {
        ...
        if (!bMultiSelect) {
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    ClientApp clientApp = model.getHub().getAO(); 
                    if (clientApp != null) onSelect(clientApp, model.getHub());
                }
            });
            pan.add(panx, BorderLayout.SOUTH);
        }
        ...
        SwingWorker<OATree, Void> sw = new SwingWorker<OATree, Void>() {
            ...
            protected OATree doInBackground() throws Exception {
                ClientAppJfc jfcClientApp = new ClientAppJfc(model);
                OATree tree = jfcClientApp.createSearchTree();
                tree.preload();
                return tree;
            }
            @Override
    ...
    protected JPanel createSearchResultsPanel() {
        ...
        final Hub<ClientApp> hub = getHub();
        OATable table = createReadOnlyTable(hub);
        OATableScrollPane spTable = new OATableScrollPane(table, 1);
    ...
    public OATable createReadOnlyTable(final Hub<ClientApp> hubClientApp) {
        Hub<ClientApp> hubx = new Hub(ClientApp.class);
        OATable table = new OATable(hubx) {
            @Override
            public void onDoubleClick() {
                if (ClientAppSearchJfc.this.getModel().getMultiSelectHub() != hubClientApp) {
                    ClientApp clientApp = hubClientApp.getAO();
                    if (clientApp != null) onSelect(clientApp, hubClientApp);
                }
            }
        };
        table.setFilterMasterHub(hubClientApp);
        table.setAllowDnD(false);
        table.setAllowSorting(true);
        table.addCounterColumn();
    
        if (bMultiSelect && getModel().getMultiSelectHub() != hubClientApp) {
    ...
    protected void createTableColumns(OATable table) {
        getClientAppJfc().createReadOnlyTableColumns(table);
    }
    public OAButton createSelectOneButton(final Hub<ClientApp> hub) {
        ...
        cmd.setToolTipText("Add selected ClientApp");
        OAButton.setup(cmd);
    ...
    public JButton createSelectManyButton() {
        ...
        cmd.setToolTipText("Add selected ClientApps");
    ...
    public OAButton createRemoveButton(Hub<ClientApp> hub) {
        OAButton cmd = new OAButton(hub, OAButton.REMOVE);
        cmd.setText("Remove");
        cmd.setup();
        cmd.setToolTipText("Remove selected ClientApp");
        return cmd;
    }
    public OAButton createRemoveAllButton(Hub<ClientApp> hub) {
        ...
        cmd.setup();
        cmd.setToolTipText("Remove all ClientApps from the select list");
        return cmd;
    }
    ...
    public ClientAppJfc getClientAppJfc() {
        if (jfcClientApp == null) {
            jfcClientApp = new ClientAppJfc(new Hub<ClientApp>(ClientApp.class));
            OAObjectModel om = jfcClientApp.getModel();
    ...
    public OAButton createGotoEditButton(final Hub<ClientApp> hub) {
        OAButton cmd = new OAButton(hub, "Details ...", Resource.getJarIcon(Resource.IMG_Goto), OAButton.GOTO) {
            @Override
            public void afterActionPerformed() {
                getClientAppJfc().getHub().setSharedHub(hub, true);
                getClientAppJfc().getEditDialog(this).setVisible(true);
    ...
    public JDialog getDialog() {
        ...
        dialog = new JDialog(win, "Client App Search", ModalityType.APPLICATION_MODAL);
        ...
        dialog.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                ClientAppSearchJfc.this.bSelected = false;
                onShowDialog();
            }
            @Override
            public void windowClosing(WindowEvent e) {
                if (!ClientAppSearchJfc.this.bSelected) {
                    ClientAppSearchJfc.this.onCancel();
                }
                onHideDialog();
            }
        });
    ...
    protected void onReset() {
        getHub().cancelSelect();
        getHub().clear();
        getModel().getClientAppSearch().reset();
    }
    
    protected void onSelect(ClientApp clientApp, Hub<ClientApp> hubFrom) {
        clientAppSelected = clientApp;
        bSelected = true;
        getHub().cancelSelect();
        getModel().onSelect(clientApp, hubFrom);
        if (dialog != null) dialog.setVisible(false);
    }
    protected void onSelect(Hub<ClientApp> hub) {
        bSelected = true;
        getHub().cancelSelect();
    ...
    protected void onSearch(final JComponent compFrom) {
        /*
        if (!getModel().getClientAppSearch().isDataEntered()) {
            String msg = "Please enter selection data.";
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom), msg, "ClientApp Search", JOptionPane.WARNING_MESSAGE);
            return;
        }
        */
        ...
        dlgWait.setStatus("Selecting ClientApps ... please wait ...");
        dlgWait.setTitle("ClientApp Search");
        ...
        if (dlgWait.wasCancelled()) {
            ...
            String msg = "No ClientApps were found.";
            try {
                sw.get();
            }
            catch (Exception e) {
            ...
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                msg, "ClientApp Search",
                JOptionPane.WARNING_MESSAGE
            );
            if (compFocus != null) compFocus.requestFocus();
        }
        else {
            int max = getModel().getClientAppSearch().getMaxResults();
            if (max > 0 && getHub().getSize() == max) {
                String msg = "Selected the first (max="+max+") Client Apps.";
                JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                    msg, "ClientApp Search",
    ...
    public ClientApp getSelected() {
        return clientAppSelected;
    }
