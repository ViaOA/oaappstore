...
public class RunningAppSearchJfc {
    private static Logger LOG = Logger.getLogger(RunningAppSearchJfc.class.getName());
    
    private RunningAppSearchModel model;
    private JDialog dialog;
    protected RunningAppJfc jfcRunningApp;
    
    protected boolean bShowResults;
    protected boolean bMultiSelect;
    protected RunningApp runningAppSelected;
    protected boolean bSelected;
    
    public RunningAppSearchJfc() {
        this(null, true, false);
    }
    public RunningAppSearchJfc(RunningAppSearchModel model) {
        this(model, true, false);
    }
    public RunningAppSearchJfc(RunningAppSearchModel model, boolean bShowResults, boolean bMultiSelect) {
        this.model = model;
        this.bShowResults = bShowResults;
        this.bMultiSelect = bMultiSelect;
    }
    public RunningAppSearchJfc(boolean bShowResults) {
        this(null, bShowResults, false);
    }
    
    public RunningAppSearchModel getModel() {
        if (model == null) {
            model = new RunningAppSearchModel();
            model.getRunningAppSearch().setMaxResults(100); // default for UI searches
        }
        return model;
    }
    ...
    public JPanel createMainPanel() {
        ...
        if (cnt > 1) {
            ...
            Icon icon = Resource.getJarIcon("runningApp.gif");
            if (panSearch != null) {
    ...
    public JPanel createSearchFromPanel() {
        final Hub<RunningApp> hub = getModel().getSearchFromHub();
        ...
        if (bMultiSelect) table.setMultiSelectControlKey(true);
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    RunningApp runningApp = hub.getAO();
                    if (runningApp != null) onSelect(runningApp, hub);
                }
            });
        }
        return pan;
    ...
    public JPanel createMultiSelectPanel() {
        final Hub<RunningApp> hub = getModel().getMultiSelectHub();
        JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        JLabel lbl = new JLabel("RunningApps to add");
    ...
    public JPanel createSearchTreePanel() {
        // no tree properties defined in model, for RunningApp
        if (true || false) return null;
        // if (!bShowResults) return null;
        final JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        final RunningAppModel model = new RunningAppModel();
        if (bMultiSelect) {
        ...
        if (!bMultiSelect) {
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    RunningApp runningApp = model.getHub().getAO(); 
                    if (runningApp != null) onSelect(runningApp, model.getHub());
                }
            });
            pan.add(panx, BorderLayout.SOUTH);
        }
        ...
        SwingWorker<OATree, Void> sw = new SwingWorker<OATree, Void>() {
            ...
            protected OATree doInBackground() throws Exception {
                RunningAppJfc jfcRunningApp = new RunningAppJfc(model);
                OATree tree = jfcRunningApp.createSearchTree();
                tree.preload();
                return tree;
            }
            @Override
    ...
    protected JPanel createSearchResultsPanel() {
        ...
        final Hub<RunningApp> hub = getHub();
        OATable table = createReadOnlyTable(hub);
        OATableScrollPane spTable = new OATableScrollPane(table, 1);
    ...
    public OATable createReadOnlyTable(final Hub<RunningApp> hubRunningApp) {
        Hub<RunningApp> hubx = new Hub(RunningApp.class);
        OATable table = new OATable(hubx) {
            @Override
            public void onDoubleClick() {
                if (RunningAppSearchJfc.this.getModel().getMultiSelectHub() != hubRunningApp) {
                    RunningApp runningApp = hubRunningApp.getAO();
                    if (runningApp != null) onSelect(runningApp, hubRunningApp);
                }
            }
        };
        table.setFilterMasterHub(hubRunningApp);
        table.setAllowDnD(false);
        table.setAllowSorting(true);
        table.addCounterColumn();
    
        if (bMultiSelect && getModel().getMultiSelectHub() != hubRunningApp) {
    ...
    protected void createTableColumns(OATable table) {
        getRunningAppJfc().createReadOnlyTableColumns(table);
    }
    public OAButton createSelectOneButton(final Hub<RunningApp> hub) {
        ...
        cmd.setToolTipText("Add selected RunningApp");
        OAButton.setup(cmd);
    ...
    public JButton createSelectManyButton() {
        ...
        cmd.setToolTipText("Add selected RunningApps");
    ...
    public OAButton createRemoveButton(Hub<RunningApp> hub) {
        OAButton cmd = new OAButton(hub, OAButton.REMOVE);
        cmd.setText("Remove");
        cmd.setup();
        cmd.setToolTipText("Remove selected RunningApp");
        return cmd;
    }
    public OAButton createRemoveAllButton(Hub<RunningApp> hub) {
        ...
        cmd.setup();
        cmd.setToolTipText("Remove all RunningApps from the select list");
        return cmd;
    }
    ...
    public RunningAppJfc getRunningAppJfc() {
        if (jfcRunningApp == null) {
            jfcRunningApp = new RunningAppJfc(new Hub<RunningApp>(RunningApp.class));
            OAObjectModel om = jfcRunningApp.getModel();
    ...
    public OAButton createGotoEditButton(final Hub<RunningApp> hub) {
        OAButton cmd = new OAButton(hub, "Details ...", Resource.getJarIcon(Resource.IMG_Goto), OAButton.GOTO) {
            @Override
            public void afterActionPerformed() {
                getRunningAppJfc().getHub().setSharedHub(hub, true);
                getRunningAppJfc().getEditDialog(this).setVisible(true);
    ...
    public JDialog getDialog() {
        ...
        dialog = new JDialog(win, "Running App Search", ModalityType.APPLICATION_MODAL);
        ...
        dialog.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                RunningAppSearchJfc.this.bSelected = false;
                onShowDialog();
            }
            @Override
            public void windowClosing(WindowEvent e) {
                if (!RunningAppSearchJfc.this.bSelected) {
                    RunningAppSearchJfc.this.onCancel();
                }
                onHideDialog();
            }
        });
    ...
    protected void onReset() {
        getHub().cancelSelect();
        getHub().clear();
        getModel().getRunningAppSearch().reset();
    }
    
    protected void onSelect(RunningApp runningApp, Hub<RunningApp> hubFrom) {
        runningAppSelected = runningApp;
        bSelected = true;
        getHub().cancelSelect();
        getModel().onSelect(runningApp, hubFrom);
        if (dialog != null) dialog.setVisible(false);
    }
    protected void onSelect(Hub<RunningApp> hub) {
        bSelected = true;
        getHub().cancelSelect();
    ...
    protected void onSearch(final JComponent compFrom) {
        /*
        if (!getModel().getRunningAppSearch().isDataEntered()) {
            String msg = "Please enter selection data.";
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom), msg, "RunningApp Search", JOptionPane.WARNING_MESSAGE);
            return;
        }
        */
        ...
        dlgWait.setStatus("Selecting RunningApps ... please wait ...");
        dlgWait.setTitle("RunningApp Search");
        ...
        if (dlgWait.wasCancelled()) {
            ...
            String msg = "No RunningApps were found.";
            try {
                sw.get();
            }
            catch (Exception e) {
            ...
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                msg, "RunningApp Search",
                JOptionPane.WARNING_MESSAGE
            );
            if (compFocus != null) compFocus.requestFocus();
        }
        else {
            int max = getModel().getRunningAppSearch().getMaxResults();
            if (max > 0 && getHub().getSize() == max) {
                String msg = "Selected the first (max="+max+") Running Apps.";
                JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                    msg, "RunningApp Search",
    ...
    public RunningApp getSelected() {
        return runningAppSelected;
    }
