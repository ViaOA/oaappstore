...
public class AppUserJfcBase implements OAModelJfcInterface {
    ...
    protected static final String CARD_ApplicationTypes = "ApplicationTypes";
    protected static final String CARD_AppUserLogins = "AppUserLogins";
    protected static final String CARD_ClientApps = "ClientApps";
    protected static final String CARD_MergeServerApplications = "MergeServerApplications";
    protected static final String CARD_SingleApps = "SingleApps";
    protected int TAB_AppUserLogins = -1;
    protected int TAB_ApplicationTypes = -1;
    protected int TAB_MergeServerApplications = -1;
    ...
    protected ApplicationTypeJfc jfcApplicationTypes;
    protected AppUserLoginJfc jfcAppUserLogins;
    protected ClientAppJfc jfcClientApps;
    protected ServerApplicationJfc jfcMergeServerApplications;
    ...
    public OATreeNode createTreeNode(String leadingPropertyPath, Hub<AppUser> hubRoot, boolean bIncludeChildren, OATreeNode ... parentNodes) {
        ...
        OATreeNode node = new OATreeNode(OAString.cpp(leadingPropertyPath, AppUser.P_DisplayName), hubRoot, getHub()) {
            ...
            public void objectSelected(Object obj) {
                super.objectSelected(obj);
                AppUserJfcBase.this.getCardPanel();
    ...
    public JPopupMenu createTreeNodePopupMenu(OATreeNode tn) {
        ...
        if (mi != null) menu.add(mi);
        mi = getClientAppsJfc().createNewMenuItem();
        if (mi != null) menu.add(mi);
        mi = getClientAppsJfc().createAddMenuItem();
        if (mi != null) menu.add(mi);
    ...
    public void createTableColumns(OATable table) {
        ...
        tc.setToolTipText("allows setting up new users");
        tc = table.addColumn("Edit Processed", 5, createEditProcessedCheckBox());
        tc.setToolTipText("allows user to edit processed data");
        tc = table.addColumn("Super Admin", 11, createSuperAdminCheckBox());
        tc.setToolTipText("gives user the permissions to change anything in the system.");
    ...
    protected void createReadOnlyTableColumns(OATable table) {
        ...
        lbl = new OACheckBoxLabel(getHub(), AppUser.P_Admin, 5);
        lbl.setToolTipText("allows setting up new users");
        tc = table.addColumn("Admin", 5, lbl);
        lbl = new OACheckBoxLabel(getHub(), AppUser.P_EditProcessed, 5);
        lbl.setToolTipText("allows user to edit processed data");
        tc = table.addColumn("Edit Processed", 5, lbl);
        lbl = new OACheckBoxLabel(getHub(), AppUser.P_SuperAdmin, 5);
        lbl.setToolTipText("gives user the permissions to change anything in the system.");
        tc = table.addColumn("Super Admin", 11, lbl);
    ...
    protected void addDownloadProperties(DownloadDialog dd) {
        ...
        dd.addProperty("editProcessed", AppUser.P_EditProcessed);
        dd.addProperty("superAdmin", AppUser.P_SuperAdmin);
    ...
    public JPanel createEditOnePanel(JTabbedPane tp) {
        ...
        // cardPanel.add(getApplicationTypesJfc().getCardPanel(), CARD_ApplicationTypes); // this will be created when needed by showCardPanel(..)
        ...
        // cardPanel.add(getMergeServerApplicationsJfc().getCardPanel(), CARD_MergeServerApplications); // this will be created when needed by showCardPanel(..)
    ...
    public JPanel createEditPanel(JTabbedPane tabbedPane, final boolean bUseCombinedDetail) {
        ...
        lbl = new JLabel("Disable Auto Login:");
        gc.anchor = gc.WEST;
        panel.add(lbl, gc);
        gc.anchor = gc.NORTHWEST;
        chk = createDisableAutoLoginCheckBox();
        if (getModel().getViewOnly()) chk.getController().setViewOnly(true);
        chk.setLabel(lbl);
        gc.gridwidth = gc.REMAINDER;
        gc.fill = gc.HORIZONTAL;
        ...
        icon = Resource.getJarIcon("applicationType.gif");
        icon = new ScaledImageIcon(icon, 32, 20);
        pan = getApplicationTypesJfc().createReadOnlyTablePanel();
        if (getModel().getApplicationTypesModel().getCreateUI()) {
            this.TAB_ApplicationTypes = tabbedPane.getTabCount();
            tabbedPane.addTab("Application Types", icon, pan, null);
        }
        ...
        tabbedPane.addChangeListener(new ChangeListener() {
            ...
            public void stateChanged(ChangeEvent e) {
                if (panThis != null) return;
                if (AppUserJfcBase.this.TAB_MergeServerApplications == 0) return;
                final JTabbedPane tp = (JTabbedPane) e.getSource();
                if (tp.getSelectedIndex() != AppUserJfcBase.this.TAB_MergeServerApplications) return;
                SwingWorker<Void, Void> sw = new SwingWorker<Void, Void>() {
                    @Override
                    protected Void doInBackground() throws Exception {
                        try {
                            if (bUseCombinedDetail) panThis = getMergeServerApplicationsJfc().createCombinedPanel();
                            else panThis = getMergeServerApplicationsJfc().createTablePanel();
                        }
                        catch (Exception e) {
                            ex = e;
                        }
                    ...
                    protected void done() {
                        ...
                        if (panThis == null) panThis = new JPanel();
                        tp.setComponentAt(AppUserJfcBase.this.TAB_MergeServerApplications, panThis);
                    }
                };
                sw.execute();
            }
        });
        if (getModel().getMergeServerApplicationsModel().getCreateUI()) {
            if ((this.TAB_MergeServerApplications = tabbedPane.getTabCount()) == 0) {
                JPanel panThis;
                if (bUseCombinedDetail) panThis = getMergeServerApplicationsJfc().createCombinedPanel();
                else panThis = getMergeServerApplicationsJfc().createTablePanel();
                tabbedPane.setComponentAt(AppUserJfcBase.this.TAB_MergeServerApplications, panThis);
            }
            else {
                tabbedPane.addTab("Available Apps", icon, new JLabel("loading ...", Resource.getJarIcon("wait.png"), JLabel.CENTER), "Server Applications");
            }
        }
        ...
        tabbedPane.addChangeListener(new ChangeListener() {
            ...
            public void stateChanged(ChangeEvent e) {
                if (panThis != null) return;
                if (AppUserJfcBase.this.TAB_ClientApps == 0) return;
                final JTabbedPane tp = (JTabbedPane) e.getSource();
                if (tp.getSelectedIndex() != AppUserJfcBase.this.TAB_ClientApps) return;
                SwingWorker<Void, Void> sw = new SwingWorker<Void, Void>() {
                    @Override
                    protected Void doInBackground() throws Exception {
                        try {
                    ...
                    protected void done() {
                        ...
                        if (panThis == null) panThis = new JPanel();
                        tp.setComponentAt(AppUserJfcBase.this.TAB_ClientApps, panThis);
                    }
                };
                sw.execute();
            }
        ...
        if (getModel().getClientAppsModel().getCreateUI()) {
            if ((this.TAB_ClientApps = tabbedPane.getTabCount()) == 0) {
                ...
                tabbedPane.setComponentAt(AppUserJfcBase.this.TAB_ClientApps, panThis);
            }
            else {
        ...
        tabbedPane.addChangeListener(new ChangeListener() {
            ...
            public void stateChanged(ChangeEvent e) {
                if (panThis != null) return;
                if (AppUserJfcBase.this.TAB_SingleApps == 0) return;
                final JTabbedPane tp = (JTabbedPane) e.getSource();
                if (tp.getSelectedIndex() != AppUserJfcBase.this.TAB_SingleApps) return;
                SwingWorker<Void, Void> sw = new SwingWorker<Void, Void>() {
                    @Override
                    protected Void doInBackground() throws Exception {
                        try {
                    ...
                    protected void done() {
                        ...
                        if (panThis == null) panThis = new JPanel();
                        tp.setComponentAt(AppUserJfcBase.this.TAB_SingleApps, panThis);
                    }
                };
                sw.execute();
            }
    ...
    public OACheckBox createAdminCheckBox() {
        ...
        // chk.setText("Admin")
        chk.setToolTipText("allows setting up new users");
        return chk;
    }
    
    public OACheckBox createSuperAdminCheckBox() {
        OACheckBox chk = new OACheckBox(getHub(), AppUser.P_SuperAdmin);
        // chk.setText("Super Admin")
        chk.setToolTipText("gives user the permissions to change anything in the system.");
        return chk;
    }
    ...
    public OACheckBox createEditProcessedCheckBox() {
        ...
        chk.setToolTipText("allows user to edit processed data");
        return chk;
    }
    ...
    public OACheckBox createDisableAutoLoginCheckBox() {
        OACheckBox chk = new OACheckBox(getHub(), AppUser.P_DisableAutoLogin);
        // chk.setText("Disable Auto Login")
        return chk;
    }
    
    public OATextField createFullNameTextField() {
        OATextField txt = new OATextField(getHub(), AppUser.P_FullName, 23);
    ...
    public OATextField createDisplayNameTextField() {
        OATextField txt = new OATextField(getHub(), AppUser.P_DisplayName, 28);
        txt.getController().getEnabledChangeListener().addAlwaysFalse();
        OAJfcUtil.initializeCalcTextField(txt);
        txt.setMinimumColumns(0);
        txt.setMaximumColumns(50);
    ...
    public JButton createServerCommand() {
        ...
        Hub h;
        if (b) {
            cmd = new OAButton(getModel().getServerHub(), OAButton.CLEARAO);
            cmd.setText("Clear");
            ((OAButton)cmd).getController().setViewOnly(getModel().getViewOnly());
            mscmd.addButton(cmd);
        }
    ...
    public ServerJfc getServerJfc() {
        ...
        jfcServer = new ServerJfc(getModel().getServerModel()) {
            ...
            protected ServerSearchJfc getSearchJfc() {
                ...
                AppUserJfcBase.this.getModel().getServerSearchModel().getServerSearch().setMaxResults(1000);
                jfcSearch = new ServerSearchJfc(AppUserJfcBase.this.getModel().getServerSearchModel());
                return jfcSearch;
            }
            @Override
    ...
    public ApplicationTypeJfc getApplicationTypesJfc() {
        if (jfcApplicationTypes == null) {
            jfcApplicationTypes = createApplicationTypesJfc();
        }
        return jfcApplicationTypes;
    }
    public ApplicationTypeJfc createApplicationTypesJfc() {
        return createApplicationTypesJfc(true);
    }
    public ApplicationTypeJfc createApplicationTypesJfc(final boolean bIsEmbedded) {
        jfcApplicationTypes = new ApplicationTypeJfc(getModel().getApplicationTypesModel()) {
            @Override
            protected ApplicationTypeSearchJfc getSearchJfc() {
                if (jfcSearch != null) return jfcSearch;
                ApplicationTypeSearchModel model = AppUserJfcBase.this.getModel().getApplicationTypesSearchModel();
                jfcSearch = new ApplicationTypeSearchJfc(model, true, true);
                return jfcSearch;
            }
            @Override
            public JPanel getCardPanel() {
            ...
            public void showCardPanel(String name) {
                if (!bIsEmbedded) {
                    ...
                    AppUserJfcBase.this.showCardPanel(name);
                    return;
                }
                if (name.equals(ApplicationTypeJfc.CARD_List)) {
                    AppUserJfcBase.this.showCardPanel(CARD_Edit);
                    if (AppUserJfcBase.this.TAB_ApplicationTypes >= 0) {
                        AppUserJfcBase.this.getTabbedPane().setSelectedIndex(AppUserJfcBase.this.TAB_ApplicationTypes);
                    }
                }
                else if (name.equals(ApplicationTypeJfcBase.CARD_Edit)) {
                    AppUserJfcBase.this.showCardPanel(CARD_Edit);
                    if (AppUserJfcBase.this.TAB_ApplicationTypes >= 0) {
                        AppUserJfcBase.this.getTabbedPane().setSelectedIndex(AppUserJfcBase.this.TAB_ApplicationTypes);
                    }
                    ...
                    AppUserJfcBase.this.showCardPanel(CARD_ApplicationTypes);
                    super.showCardPanel(name);
                }
            }
            public JButton createGoBackButton() {
            ...
            public OAButton createAddButton() {
               if (!getModel().getAllowAdd()) return null;
               final ApplicationTypeModel modelApplicationType = AppUserJfcBase.this.getModel().getApplicationTypesSelectFromModel();
               ApplicationTypeJfcBase jfcApplicationType = new ApplicationTypeJfcBase(modelApplicationType) {
                   @Override
                   protected ApplicationTypeSearchJfc getSearchJfc() {
                       if (jfcSearch != null) return jfcSearch;
                          jfcSearch = new ApplicationTypeSearchJfc(AppUserJfcBase.this.getModel().getApplicationTypesSearchModel(), true, true) {
                	        @Override
                           ...
                           protected void onSelect(Hub<ApplicationType> hub) {
                               super.onSelect(hub);
                               AppUserJfcBase.this.getModel().getApplicationTypes().add(hub);
                           }
                       };
                       return jfcSearch;
                   }
                ...
                final OAButton cmdOk = new OAButton(AppUserJfcBase.this.getHub(), OAButton.OK) {
                    ...
                    protected boolean onActionPerformed() {
                        AppUserJfcBase.this.getModel().getApplicationTypes().add(modelApplicationType.getMultiSelectHub());
                        return true;
                    }
                };
                cmdOk.setText("Ok");
                ...
                final OAButton cmdCancel = new OAButton(AppUserJfcBase.this.getHub(), OAButton.CANCEL);
                cmdCancel.setManual(true);
                cmdCancel.setText("Cancel");
                panCommand.add(cmdCancel);
                OAButton cmdAdd = new OAButton(getHub(), OAButton.ADD_MANUAL);
                cmdAdd.setText("Add ...");
                cmdAdd.setToolTipText("Add to list of Application Types");
        ...
        jfcApplicationTypes.setLevel(getLevel()+1);
        OAModelJfcUtil.setParent(jfcApplicationTypes, this);
        return jfcApplicationTypes;
    }
    ...
    public AppUserLoginJfc createAppUserLoginsJfc() {
        return createAppUserLoginsJfc(true);
    }
    public AppUserLoginJfc createAppUserLoginsJfc(final boolean bIsEmbedded) {
        ...
        jfcClientApps = new ClientAppJfc(getModel().getClientAppsModel()) {
            ...
            public void showCardPanel(String name) {
                ...
                if (name.equals(ClientAppJfc.CARD_List)) {
                    AppUserJfcBase.this.showCardPanel(CARD_Edit);
                    if (AppUserJfcBase.this.TAB_ClientApps >= 0) {
                        AppUserJfcBase.this.getTabbedPane().setSelectedIndex(AppUserJfcBase.this.TAB_ClientApps);
                    }
                }
                    ...
                    AppUserJfcBase.this.showCardPanel(CARD_ClientApps);
                    super.showCardPanel(name);
                }
            }
            public JButton createGoBackButton() {
            ...
            public OAButton createDeleteButton() {
                ...
                cmd.setText("Remove ...");
                cmd.setConfirmMessage("OK to remove this Server Application from this App User?");
                cmd.setUseSwingWorker(false);
                cmd.setToolTipText("remove this Server Application from this App User?");
                return cmd;
            }
            @Override
            public OAButton createAddButton() {
                if (!getModel().getAllowAdd()) return null;
                final ServerApplicationModel modelServerApplication = AppUserJfcBase.this.getModel().getClientAppsPickFromServerApplicationModel();
                ServerApplicationJfcBase jfcServerApplication = new ServerApplicationJfcBase(modelServerApplication) {
                    @Override
                    protected ServerApplicationSearchJfc getSearchJfc() {
                        if (jfcSearch != null) return jfcSearch;
                        jfcSearch = new ServerApplicationSearchJfc(AppUserJfcBase.this.getModel().getClientAppsPickFromServerApplicationSearchModel(), true, true) {
                            @Override
                            protected void onShowDialog() {
                                getHub().clear();
                            }
                            ...
                            protected void onSelect(Hub<ServerApplication> hub) {
                                super.onSelect(hub);
                                HubAutoAdd<ClientApp, ServerApplication> ham = new HubAutoAdd(AppUserJfcBase.this.getModel().getClientApps(), ClientApp.P_ServerApplication, hub, true);
                                ham.update();
                            }
                        };
                        return jfcSearch;
                ...
                cmdOk.setText("Ok");
                panCommand.add(cmdOk);
                OAButton cmdSearch = jfcServerApplication.createSearchButton();
                if (cmdSearch != null) {
                    cmdSearch.setText("Search ...");
                    panCommand.add(cmdSearch);
                }
                ...
                OAButton cmdAdd = new OAButton(getHub(), OAButton.ADD_MANUAL);
                cmdAdd.setText("Add ...");
                cmdAdd.setToolTipText("Select a Server Application to add to Client Apps list.");
                final OAPopup popup = new OAPopup(panPopup, cmdAdd) {
                    protected void showPopup(Component comp) {
                        table.setPreferredRows(Math.max(5, Math.min(14, modelServerApplication.getHub().getSize())));
                        super.showPopup(comp);
                    }
                };
                popup.addCloseCommand(cmdOk);
    ...
    public ServerApplicationJfc getMergeServerApplicationsJfc() {
        if (jfcMergeServerApplications == null) {
            jfcMergeServerApplications = createMergeServerApplicationsJfc();
        }
    ...
    public ServerApplicationJfc createMergeServerApplicationsJfc(final boolean bIsEmbedded) {
        jfcMergeServerApplications = new ServerApplicationJfc(getModel().getMergeServerApplicationsModel()) {
            ...
            public void showCardPanel(String name) {
                ...
                if (name.equals(ServerApplicationJfc.CARD_List)) {
                    AppUserJfcBase.this.showCardPanel(CARD_Edit);
                    if (AppUserJfcBase.this.TAB_MergeServerApplications >= 0) {
                        AppUserJfcBase.this.getTabbedPane().setSelectedIndex(AppUserJfcBase.this.TAB_MergeServerApplications);
                    }
                }
                    ...
                    AppUserJfcBase.this.showCardPanel(CARD_MergeServerApplications);
                    super.showCardPanel(name);
                }
            }
            public JButton createGoBackButton() {
        ...
        jfcMergeServerApplications.setLevel(getLevel()+1);
        OAModelJfcUtil.setParent(jfcMergeServerApplications, this);
        return jfcMergeServerApplications;
    }
        ...
        jfcSingleApps = new SingleAppJfc(getModel().getSingleAppsModel()) {
            ...
            public void showCardPanel(String name) {
                ...
                if (name.equals(SingleAppJfc.CARD_List)) {
                    AppUserJfcBase.this.showCardPanel(CARD_Edit);
                    if (AppUserJfcBase.this.TAB_SingleApps >= 0) {
                        AppUserJfcBase.this.getTabbedPane().setSelectedIndex(AppUserJfcBase.this.TAB_SingleApps);
                    }
                }
                    ...
                    AppUserJfcBase.this.showCardPanel(CARD_SingleApps);
                    super.showCardPanel(name);
                }
            }
            public JButton createGoBackButton() {
            ...
            public OAButton createDeleteButton() {
                ...
                if (cmd == null) return null;
                cmd.setText("Remove ...");
                cmd.setConfirmMessage("OK to remove this Application Type from this App User?");
                cmd.setUseSwingWorker(false);
                cmd.setToolTipText("remove this Application Type from this App User?");
                return cmd;
            }
            @Override
            public OAButton createAddButton() {
                if (!getModel().getAllowAdd()) return null;
                final ApplicationTypeModel modelApplicationType = AppUserJfcBase.this.getModel().getSingleAppsPickFromApplicationTypeModel();
                ApplicationTypeJfcBase jfcApplicationType = new ApplicationTypeJfcBase(modelApplicationType) {
                    @Override
                    protected ApplicationTypeSearchJfc getSearchJfc() {
                        if (jfcSearch != null) return jfcSearch;
                        jfcSearch = new ApplicationTypeSearchJfc(AppUserJfcBase.this.getModel().getSingleAppsPickFromApplicationTypeSearchModel(), true, true) {
                            @Override
                            protected void onShowDialog() {
                                getHub().clear();
                            }
                            ...
                            protected void onSelect(Hub<ApplicationType> hub) {
                                super.onSelect(hub);
                                HubAutoAdd<SingleApp, ApplicationType> ham = new HubAutoAdd(AppUserJfcBase.this.getModel().getSingleApps(), SingleApp.P_ApplicationType, hub, true);
                                ham.update();
                            }
                        };
                        return jfcSearch;
                ...
                OAButton cmdAdd = new OAButton(getHub(), OAButton.ADD_MANUAL);
                cmdAdd.setText("Add ...");
                cmdAdd.setToolTipText("Select an Application Type to add to Single Apps list.");
                final OAPopup popup = new OAPopup(panPopup, cmdAdd) {
                    protected void showPopup(Component comp) {
                        table.setPreferredRows(Math.max(5, Math.min(14, modelApplicationType.getHub().getSize())));
                        super.showPopup(comp);
    ...
    public void showCardPanel(String name) {
        ...
        if (name.equalsIgnoreCase(CARD_ApplicationTypes)) {
            if ( !OAArray.contains(cardPanel.getComponents(), getApplicationTypesJfc().getCardPanel()) ) {
                cardPanel.add(getApplicationTypesJfc().getCardPanel(), CARD_ApplicationTypes);
            }
        }
        else if (name.equalsIgnoreCase(CARD_AppUserLogins)) {
        ...
        else if (name.equalsIgnoreCase(CARD_ClientApps)) {
        ...
        else if (name.equalsIgnoreCase(CARD_MergeServerApplications)) {
            if ( !OAArray.contains(cardPanel.getComponents(), getMergeServerApplicationsJfc().getCardPanel()) ) {
                cardPanel.add(getMergeServerApplicationsJfc().getCardPanel(), CARD_MergeServerApplications);
            }
        }
